...
public class PToECPointTest {
    ...
    @Test
    @DisplayName("PToECPoint calls hash method hkey")
    public void correctMethodCalled(){
        serverLogic.setup(cryptoSystem);

        serverLogic.pToECPoint(input);

        verify(cryptoSystem, times(1)).hkey(input);
    }

    @Test
    @DisplayName("PToECPoint returns the expected ECPoint")
    public void ReturnsExpected(){
        ...
        var actual = serverLogic.pToECPoint(input);
